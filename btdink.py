...
# This flag stops the torrent when set. //dink
mainkillflag = Event()
...
def hours(n):
    ...
    if h > 0:
        if h == 1:
            ...
            return '%d hours %02d min %02d sec' % (h, m, sec)
    else:
...
class HeadlessDisplayer:
    ...
    def display(self, dict):
        if self.last_update_time + 0.4 > time() and dict.get('fractionDone') not in (0.0, 1.0) and not dict.has_key('activity'):
            return
#        print 'lut', self.last_update_time
        self.last_update_time = time()
#        print 'tnow', self.last_update_time
        ...
        if dict.has_key('upTotal'):
            self.upTotal = '%.1f MB' % (dict['upTotal'])
        if dict.has_key('downTotal'):
            self.downTotal = '%.1f MB' % (dict['downTotal'])
...
#            print 'ERROR:\n' + err + '\n'
        ...
        print 'progress       ', self.percentDone, self.progress
...
def run(params):
    ...
    h = HeadlessDisplayer()
    download(params, h.chooseFile, h.display, h.finished, h.error, mainkillflag, cols, h.newpath)
    if not h.done:
        h.failed()
